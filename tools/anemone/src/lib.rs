...
mod py {
    ...
    /// We must initialise *our* rust-side statics (main-thread handler, etc.)
    /// before touching any BV; otherwise the first call segfaults on empty
    /// state. `headless::init` is idempotent and returns `AlreadyManaged`
    /// when something else (the python `binaryninja` module) already owns
    /// the C-side core; both outcomes are fine.
    static INIT_RUST_BINJA: std::sync::Once = std::sync::Once::new();
    fn ensure_binja_inited() {
        INIT_RUST_BINJA.call_once(|| {
            let _ = binaryninja::headless::init();
        });
    }
