#!/usr/bin/env python3
# Thin wrapper over agents/common/pipeline.py - signer's per-fn agent
# fn + log line are the only signer-specific bits.
...
from signer import sign_function
...
def _format_done(rec):
    ...
    return (f"[done ] {rec.name}  [{tag}]  decl={rec.submitted_decl!r}  "
...
SPEC = StageSpec(
    name="signer",
    out_bndb_suffix=".signed.bndb",
    description="Recover Rust function signatures in parallel.",
    run_one=_run_one,
    format_done=_format_done,
    add_extra_args=_add_args,
    extra_run_kwargs=_extra_run_kwargs,
    write_namespaces={"signer"},
)
